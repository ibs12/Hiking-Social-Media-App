...
// icons for the Autocomplete search bar
...
const Autocomplete = ({ suggestions, selectAutocomplete }) => {
	...
	const onChange = (e) => {
		...
		if (suggestions) {
			filteredSuggestions = suggestions.filter(
				(suggestion) => suggestion.attributes.username.toLowerCase().indexOf(userInput.toLowerCase()) > -1
			);
		}
	...
	if (showSuggestions && userInput) {
		if (filteredSuggestions.length) {
			...
			suggestionsListComponent = (
				<div className="autocomplete">
					<em>{language === "en" ? " No suggestions, you're on your own!" : "No hay sugerencias."} </em>
				</div>
			);
		}
	} else {
		...
		<>
			<div className="autocomplete-input-container">
				<input
					...
					onKeyDown={onKeyDown}
					value={userInput}
					placeholder={language === "en" ? "Find a friend..." : "Busca un amigo"}
				/>
